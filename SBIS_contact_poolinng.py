...
def contact_polling():
    try:
        ...
        i = 0
        while True:
            # Обновляет динамический список клиентов, имитирует наведение мышки и кликает
            #clients = global_xpath_find_all("//div[@class='crm_ClientMain__item crm_ClientMain__item ws-ellipsis ws-flexbox ws-flex-column']")
            # Костыль. Не удалять. При втором поиске элементы позиционируются правильно
            clients = global_xpath_find_all("//div[@class='crm_ClientMain__item crm_ClientMain__item ws-ellipsis ws-flexbox ws-flex-column']")
            print(len(clients))
            move_and_click(clients[i])
            i += 1
            print('click ' + str(i))
            # Ждёт, пока загрузится контакт до кликабельного состояния
            wait.until(EC.element_to_be_clickable((By.XPATH, "//span[@data-qa='cd-ItemTemplate__assigneeName']")))

            # Сохраняет имя компании и нажимает кнопки раскрытия контакта, если они есть
            client_name = global_xpath_find("//div[@class='controls-Tabs__item_overflow']").text
            print(client_name)
